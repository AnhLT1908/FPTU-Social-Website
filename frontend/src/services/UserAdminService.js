...
export const getListUser = async (page = 1, limit = 5, status = '', email = '', username = '') => {
    try {
        const response = await axios.get('http://localhost:5173/api/v1/users/list', { // Updated endpoint
...
export const toggleUserStatus = async (userId) => {
    try {
        const response = await axios.patch(`http://localhost:5173/api/v1/users/${userId}/toggle-active`, {}, {
            headers: getAuthHeader(),
        });
        return response.data;
    } catch (error) {
