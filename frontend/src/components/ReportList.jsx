...
const ReportList = () => {
    ...
    useEffect(() => {
        const fetchReports = async () => {
            try {
                const response = await api.get('http://localhost:5173/api/v1/reports/v2');
    ...
    return (
        <div>
            ...
            {reports.map(report => (
                <div key={report._id} style={{ border: '1px solid #ddd', margin: '10px', padding: '10px' }}>
                    <p><strong>User:</strong> {report.userId?.username || "N/A"}</p>
                    <p><strong>Entity:</strong> {report.entityType}</p>
