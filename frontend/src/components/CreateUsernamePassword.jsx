...
const CreateUPForm = () => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        const responseUsername = await fetch(
          "http://localhost:9999/api/v1/users/check-username",
          {
            method: "POST",
        ...
        const responseStuCode = await fetch(
          "http://localhost:9999/api/v1/users/check-student-code",
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (!responseUsername.ok || !responseStuCode.ok) {
          ...
          setErrors({
            form:
              errorData.message || "Username or student code already taken.",
          });
          return;
        }
  ...
  const validateForm = () => {
    ...
    if (!studentCode || !/^(HE|HA|HS)\d{6}$/.test(studentCode))
